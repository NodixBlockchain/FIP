...
class TmpCatModel extends Model
{
	...
	public function validate_tmp_cat($catHash, $userhash)
	{
		...
		file_put_contents($dst, $newCatHash.';'.$userhash.';'.$data['created-time'].';'.$data['parent-first-name'].';'.$data['parent-last-name'].';'.$data['fb-name'].';'.$data['cat-name'].';'.$data['cat-birthdate'].';'.$data['cat-birthdate-exact'].';'.$data['cat-gender'].';'.$data['cat-fixed'].';'.$data['cat-breed'].$this->save-diag($data['cat-diagnosis']).';'.$data['cat-diagnosis-date']."\n");
